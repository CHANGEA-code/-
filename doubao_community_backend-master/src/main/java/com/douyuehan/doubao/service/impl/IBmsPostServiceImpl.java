...
public class IBmsPostServiceImpl extends ServiceImpl<BmsTopicMapper, BmsPost> implements IBmsPostService {
    ...
    private void setTopicTags(Page<PostVO> iPage) {
        iPage.getRecords().forEach(topic -> {
            List<BmsTopicTag> topicTags = IBmsTopicTagService.selectByTopicId(topic.getId()); //通过文章id获取（文章标签列表）
            if (!topicTags.isEmpty()) {
                List<String> tagIds = topicTags.stream().map(BmsTopicTag::getTagId).collect(Collectors.toList());//获取文章对应的所有标签id
                List<BmsTag> tags = bmsTagMapper.selectBatchIds(tagIds);    //通过标签id获取标签实体
                topic.setTags(tags);
            }
        });
    }
